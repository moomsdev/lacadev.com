...
<div class="page-listing journal">
    <div class="container">
		...
		$title = sprintf(__('%s', 'laca'), get_search_query());
		if ( is_search() ) :
			echo '<h1 class="title-single text-center my-5 text-uppercase"> Tìm kiếm </h1>';
			echo '<h4 class="title-result text-center mb-5"> '. $total_results .' '. __('kết quả cho từ khóa', 'laca') .' "'. $title .'" </h4>';
		endif;
		?>
        ...
        if (have_posts()) :
            ...
            echo '<h3> '. __('Không có bài viết liên quan tới từ khóa tìm kiếm', 'laca') .' </h3>';
        endif;
        thePagination();
        ?>
    </div>
