...
! ( function () {
	...
	function h() {
		...
		! s &&
			...
			( i = o.scrollHeight ),
			( c = document.compatMode.indexOf( 'CSS' ) >= 0 ? l : o ),
			( e = o ),
			a.keyboardSupport && M( 'keydown', y ),
			top != self
				? ( n = ! 0 )
				: Q &&
				  f < i &&
				  ( o.offsetHeight <= f || l.offsetHeight <= f ) &&
				  ( ( ( u =
						document.createElement(
							'div'
						) ).style.cssText = `position:absolute; z-index:-10000; top:0; left:0; right:0; height:${ c.scrollHeight }px` ),
				  ...
				  c.offsetHeight <= f &&
						( ( ( f =
							document.createElement( 'div' ) ).style.clear =
							'both' ),
						o.appendChild( f ) ) ) );
	}
	let p = [],
	...
	function w( e, t, r ) {
		...
		( o = ( o = t ) > 0 ? 1 : -1 ),
			...
			( i.x === o && i.y === l ) ||
				( ( i.x = o ), ( i.y = l ), ( p = [] ), ( _ = 0 ) ),
			a.accelerationMax != 1 &&
				( ( l = Date.now() - _ ) < a.accelerationDelta &&
					( n = ( 1 + 50 / l ) / 2 ) > 1 &&
					( ( t *= n = Math.min( n, a.accelerationMax ) ),
					( r *= n ) ),
				( _ = Date.now() ) ),
			p.push( {
				x: t,
	...
	function v( t ) {
		if (
			...
			( function e() {
				for (
					let t = event.clientX,
						r = event.clientY,
						o = document.getElementsByTagName( 'pre' ),
						l = 0;
					l < o.length;
					l++
				) {
					const a = o[ l ].getBoundingClientRect();
					if (
						t >= a.left &&
						t <= a.right &&
						r >= a.top &&
		...
		let l = -t.wheelDeltaX || t.deltaX || 0,
		...
		return (
			d &&
				( t.wheelDeltaX &&
					Y( t.wheelDeltaX, 120 ) &&
					( l = -(
						( t.wheelDeltaX / Math.abs( t.wheelDeltaX ) ) *
						120
					) ),
				t.wheelDeltaY &&
					Y( t.wheelDeltaY, 120 ) &&
					( i = -(
						( t.wheelDeltaY / Math.abs( t.wheelDeltaY ) ) *
						120
					) ) ),
			l || i || ( i = -t.wheelDelta || 0 ),
			t.deltaMode === 1 && ( ( l *= 40 ), ( i *= 40 ) ),
			( r = O( r ) )
				? !! ( function ( e ) {
						if ( e ) {
							u.length || ( u = [ e, e, e ] ),
	...
	function y( t ) {
		...
		const l = /^(button|submit|radio|checkbox|file|color|image)$/i;
		if (
			t.defaultPrevented ||
			/^(textarea|select|embed|object)$/i.test( r.nodeName ) ||
			( N( r, 'input' ) && ! l.test( r.type ) ) ||
			N( e, 'video' ) ||
			( function ( e ) {
				let t = e.target,
					r = ! 1;
			...
			r.isContentEditable ||
			o ||
			( ( N( r, 'button' ) || ( N( r, 'input' ) && l.test( r.type ) ) ) &&
				t.keyCode === f.spacebar ) ||
			( N( r, 'input' ) && r.type == 'radio' && $[ t.keyCode ] )
		) {
			return ! 0;
		}
		var i = 0,
			...
			c = O( e );
		if ( ! c ) {
			return ! n || ! V || parent.keydown( t );
		}
		let u = c.clientHeight;
		switch (
			( c == document.body && ( u = window.innerHeight ), t.keyCode )
		) {
			case f.up:
				s = -a.arrowScroll;
				break;
	...
	function H( e ) {
		return (
			( e = getComputedStyle( e, '' ).getPropertyValue(
				'overflow-y'
			) ) === 'scroll' || e === 'auto'
		);
	}
	...
	function eo( e ) {
		for ( const t in e ) {
			l.hasOwnProperty( t ) && ( a[ t ] = e[ t ] );
		}
	}
	er && Z && ( M( er, v, j ), M( 'mousedown', b ), M( 'load', h ) ),
		( eo.destroy = function () {
			t && t.disconnect(),
				L( er, v ),
				L( 'mousedown', b ),
		...
		window.SmoothScrollOptions && eo( window.SmoothScrollOptions ),
		typeof define === 'function' && define.amd
			? define( () => eo )
			: typeof exports === 'object'
			? ( module.exports = eo )
			: ( window.SmoothScroll = eo );
} )(),
	SmoothScroll( {
		frameRate: 150,
		animationTime: 500,
