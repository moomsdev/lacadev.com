...
function logWebVital( name, value, unit, thresholds ) {
	...
	console.log(
		`%c${ emoji } ${ name }: ${ value.toFixed(
			2
		) }${ unit } - ${ rating } ${ rangeInfo }`,
...
if ( 'PerformanceObserver' in window ) {
	...
	new PerformanceObserver( ( entryList ) => {
		for ( const entry of entryList.getEntries() ) {
			logWebVital( 'LCP', entry.startTime, 'ms', {
				good: 2500,
				poor: 4000,
			} );
		}
	...
	window.addEventListener( 'load', () => {
		setTimeout( () => {
			...
			if ( perfData ) {
				...
				console.log(
					`  DOM Content Loaded: ${ perfData.domContentLoadedEventEnd.toFixed(
						2
					) }ms`
				);
				console.log(
					`  Page Load Complete: ${ perfData.loadEventEnd.toFixed(
						2
					) }ms`
				);
				console.log(
