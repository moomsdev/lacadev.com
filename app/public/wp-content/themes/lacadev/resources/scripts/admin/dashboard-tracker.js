...
( function () {
	...
	function initTrackers() {
		...
		document
			.querySelectorAll( '.laca-report-select' )
			.forEach( function ( select ) {
				const widget =
					select.closest( '.inside' ) || select.parentElement;
				if ( ! widget ) {
					return;
				}
				...
				select.addEventListener( 'change', function () {
					widget
						.querySelectorAll( '.laca-tab-content' )
				...
				widget
					.querySelectorAll( '.laca-paged-list' )
					.forEach( function ( list ) {
						const tab = list.closest( '.laca-tab-content' );
						const perPage =
							parseInt( tab ? tab.dataset.perPage : 5, 10 ) || 5;
	...
	function initPagination( list, perPage ) {
		...
		const prevBtn = pageRow
			? pageRow.querySelector( '.laca-page-btn.prev' )
			: null;
		const nextBtn = pageRow
			? pageRow.querySelector( '.laca-page-btn.next' )
			: null;
		const info = pageRow
			? pageRow.querySelector( '.laca-page-info' )
			: null;

		if ( ! rows.length || ! pageRow ) {
			return;
		}
