			...
			<InspectorControls>
				<PanelBody title={__('Cài đặt Marquee Brands', 'laca')}>
					<p style={{ fontSize: '12px', color: '#666', marginBottom: '15px', fontStyle: 'italic' }}>
						{__('* Bạn có thể nhấn và kéo các ô để đổi vị trí.', 'laca')}
					</p>
					{brands.map((brand, index) => (
						...
						>
							...
							<TextControl
								...
								value={brand.url}
								onChange={(val) => updateBrand(index, 'url', val)}
							/>
						</div>
					))}
					<Button isPrimary onClick={addBrand} style={{ width: '100%', justifyContent: 'center' }}>
						{__('Thêm thương hiệu', 'laca')}
					</Button>
				</PanelBody>
			</InspectorControls>
			<div {...useBlockProps({ className: 'block-marquee-editor' })}>
				<div className="marquee-preview-box" style={{ 
					padding: '2rem', 
					background: '#000', 
					color: '#fff', 
					borderRadius: '0.5rem',
					...
					<div style={{ fontWeight: 'bold', textTransform: 'uppercase', marginBottom: '1rem', opacity: 0.5 }}>
						{__('MARQUEE PREVIEW', 'laca')}
					</div>
					<div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem', justifyContent: 'center' }}>
						{brands.map((brand, index) => (
							<span key={index} style={{ border: '1px solid rgba(255,255,255,0.2)', padding: '0.5rem 1rem', borderRadius: '2rem' }}>
								{brand.name || __('(Trống)', 'laca')}
								{brand.url && <Dashicon icon="admin-links" style={{ marginLeft: '5px', fontSize: '12px' }} />}
							</span>
						))}
					</div>
				</div>
