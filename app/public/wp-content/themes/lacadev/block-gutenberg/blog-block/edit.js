...
export default function Edit( { attributes, setAttributes } ) {
	const { title, description, mode, orderBy, categoryIds, postIds, count, buttonText, buttonUrl } = attributes;
		...
		<>
			<InspectorControls>
				<PanelBody title={ __( 'Cấu hình Blog', 'laca' ) }>
					...
					{ mode === 'auto' && (
						<>
							<p><strong>{ __( 'Lọc theo chuyên mục:', 'laca' ) }</strong></p>
							{ ! categories ? (
								<Spinner />
							) : (
								<div style={{ maxHeight: '150px', overflowY: 'auto', border: '1px solid #ddd', padding: '10px', marginBottom: '15px' }}>
									{ categories.map( ( cat ) => (
										<CheckboxControl
											key={ cat.id }
											label={ cat.name }
											checked={ categoryIds.includes( cat.id ) }
											onChange={ () => setAttributes({ categoryIds: toggleItem(categoryIds, cat.id) }) }
										/>
									) ) }
								</div>
							)}
							...
							<SelectControl
								...
								options={[
									...
									{ label: __( 'Nhiều bình luận nhất (Proxy Popularity)', 'laca' ), value: 'comment_count' },
								]}
					...
					{ mode === 'manual' && (
						<div className="manual-selection">
							<p><strong>{ __( 'Chọn bài viết hiển thị:', 'laca' ) }</strong></p>
							{ ! posts ? (
								<Spinner />
							) : (
								<div style={{ maxHeight: '300px', overflowY: 'auto', border: '1px solid #ddd', padding: '10px' }}>
									{ posts.map( ( post ) => (
										<CheckboxControl
											...
											onChange={ () => setAttributes({ postIds: toggleItem(postIds, post.id) }) }
										/>
									) ) }
								</div>
							)}
					...
					<div className="blog-list">
						{ ! displayPosts ? (
							...
							displayPosts.length === 0 ? (
								<Placeholder label={ __( 'Không có bài viết', 'laca' ) }>
									{ __( 'Vui lòng chọn bài viết hoặc kiểm tra cấu hình.', 'laca' ) }
								</Placeholder>
							) : (
								displayPosts.map( ( post ) => (
									<div key={ post.id } className="blog-card">
										<div className="card-link" style={{ pointerEvents: 'none' }}>
											<div className="card-image-wrap">
												{ post.featured_media ? (
													<PostImage id={ post.featured_media } />
												) : (
													<div style={{ width: '100%', height: '220px', background: '#f0f0f0', borderRadius: '20px' }}></div>
												)}
											</div>
											<div className="card-body">
												<h3 className="card-title">{ decodeEntities( post.title.rendered ) }</h3>
												<div className="card-meta">
													<span>{ __( 'By Author', 'laca' ) }</span>
													<span>{ __( 'Just now', 'laca' ) }</span>
												</div>
											</div>
										</div>
									</div>
...
function PostImage( { id } ) {
	const media = useSelect( ( select ) => select( 'core' ).getMedia( id ), [ id ] );
	if ( ! media ) return <div className="card-image-placeholder"></div>;
	return (
		<img 
