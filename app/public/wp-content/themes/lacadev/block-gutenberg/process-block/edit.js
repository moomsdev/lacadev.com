...
export default function Edit({ attributes, setAttributes }) {
	...
	const addStep = () => {
		...
		setAttributes({ 
			steps: [...steps, { num: nextNum, title: 'New Step', desc: 'Description here' }] 
		});
	};
			...
			<InspectorControls>
				<PanelBody title={__('Cài đặt Quy trình', 'laca')}>
					...
					<p style={{ fontSize: '12px', color: '#666', marginBottom: '15px', fontStyle: 'italic' }}>
						{__('* Kéo thả các ô bên dưới để đổi thứ tự các bước.', 'laca')}
					</p>
					{steps.map((step, index) => (
						...
						>
							<div style={{ display: 'flex', justifyContent: 'flex-end', gap: '5px', marginBottom: '10px' }}>
								<Dashicon icon="move" style={{ marginRight: 'auto', opacity: 0.3 }} />
								<Button isSmall onClick={() => moveStep(index, 'up')} disabled={index === 0} icon="arrow-up-alt2" />
								<Button isSmall onClick={() => moveStep(index, 'down')} disabled={index === steps.length - 1} icon="arrow-down-alt2" />
								<Button isDestructive isSmall onClick={() => removeStep(index)} icon="no-alt" />
							</div>
							<TextControl
								label={__('Số thứ tự', 'laca')}
								value={step.num}
								onChange={(val) => updateStep(index, 'num', val)}
							/>
							<TextControl
							...
							<TextareaControl
								...
								value={step.desc}
								onChange={(val) => updateStep(index, 'desc', val)}
								rows={2}
							/>
						</div>
					))}
			...
			<section {...useBlockProps({ className: 'block-process' })}>
				<div className="container">
					...
					<div className="process-grid">
						{steps.map((step, index) => (
							<div key={index} className="process-item staggered-item">
								<RichText
									tagName="div"
								...
								<div className="process-info">
									...
									<RichText
										...
										value={step.desc}
										onChange={(val) => updateStep(index, 'desc', val)}
										placeholder={__('Mô tả bước...', 'laca')}
									/>
								</div>
							</div>
						))}
