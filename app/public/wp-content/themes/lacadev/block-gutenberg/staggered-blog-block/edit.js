		...
		<>
						...
						<>
							...
							<SelectControl
								...
								options={[
									...
									{ label: __( 'Phổ biến nhất', 'laca' ), value: 'comment_count' },
								]}
			...
			<section { ...blockProps }>
				<div className="container">
					<div className="block-header" style={{ textAlign: 'center', marginBottom: '60px' }}>
						<RichText
							tagName="h2"
						...
						{ ! displayPosts ? (
							...
							displayPosts.length === 0 ? (
								...
								displayPosts.map( ( post, index ) => (
									<div key={ post.id } className={`staggered-item ${index % 2 !== 0 ? 'staggered-item--even' : 'staggered-item--odd'}`}>
										<div className="staggered-item__content">
											<h3 className="staggered-item__title">
												{ decodeEntities( post.title.rendered ) }
											</h3>
											<div className="staggered-item__desc">
												{ decodeEntities( post.excerpt.rendered.replace(/<[^>]*>?/gm, '').substring(0, 150) ) }...
											</div>
										</div>
...
function PostImage( { id } ) {
	...
	if ( ! media ) return <div className="image-placeholder"></div>;
	return (
		<img 
