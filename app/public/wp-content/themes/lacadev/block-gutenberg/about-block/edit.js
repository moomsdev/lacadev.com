...
export default function Edit({ attributes, setAttributes, clientId }) {
	...
	return (
		<>
			<InspectorControls>
				<PanelBody title={__('Block Settings', 'laca')} initialOpen={true}>
					<TextControl
						label={__('Circle Content', 'laca')}
						value={welcomeContent}
						onChange={(value) => setAttributes({ welcomeContent: value })}
						help={__('Nội dung hiển thị trong vòng tròn xoay', 'laca')}
					/>
										...
										<div className="editor-post-featured-image">
											...
											>
												<Button
													...
													style={{ flex: 1 }}
												>
													{__('Replace Image', 'laca')}
												</Button>
												<Button
													onClick={() =>
														setAttributes({
															aboutImage: { id: 0, url: '', alt: '' },
														})
													}
													variant="secondary"
													isDestructive
			...
			<section className="block-about editor-view">
				...
				<div className="block-about__body">
					...
					<RichText
						...
						value={aboutDesc}
						onChange={(value) => setAttributes({ aboutDesc: value })}
